...
#ref: https://github.com/charlesq34/pointnet2/issues/48#issuecomment-404482115
#/usr/local/cuda-9.2/bin/nvcc -std=c++11 -c -o tf_sampling_g.cu.o tf_sampling_g.cu -O2 -DGOOGLE_CUDA=1 -x cu -Xcompiler -fPIC
#g++ -std=c++11 tf_sampling.cpp tf_sampling_g.cu.o -o tf_sampling_so.so -shared -fPIC -I ${TF_ROOT}/include -I ${TF_ROOT}/include/external/nsync/public -L ${TF_ROOT} -ltensorflow_framework -I ${CUDA_ROOT}/include -lcudart -L ${CUDA_ROOT}/lib64/ -O2 #-D_GLIBCXX_USE_CXX11_ABI=0